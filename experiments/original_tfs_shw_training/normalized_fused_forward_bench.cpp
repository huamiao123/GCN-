...
void setup_tiles(TileCfg& cfg, int d) {
  ...
  (void)d;
  for (int t = 0; t < 7; ++t) { cfg.rows[t] = 16; cfg.colsb[t] = 64; }
}
...
Times run_fused(const Graph& g, const std::vector<float>& w, const std::vector<float>& bias,
                ...
                Workspace& z, std::vector<float>& y) {
  const double t0=ms(); make_hs(g,z.hs,threads); pack_w_vnni(w,dp,z.wv); const double t1=ms();
  fused_amx(g,z.hs.data(),z.wv.data(),nullptr,y.data(),perm.data(),d,dp,threads,group_rows,bias); const double t2=ms();
...
int main(int argc,char**argv) {
  std::string fixture; int d=128,threads=1,repeats=7,warmups=2,group_rows=64;
  for(int i=1;i<argc;i+=2){if(i+1>=argc)throw std::runtime_error("missing value");std::string k=argv[i],v=argv[i+1];
    ...
    else if(k=="--repeats")repeats=std::stoi(v);else if(k=="--warmups")warmups=std::stoi(v);else if(k=="--group-rows")group_rows=std::stoi(v);else throw std::runtime_error("unknown arg "+k);}
  ...
  std::cout<<"META,n="<<g.n<<",nnz="<<g.nnz<<",d="<<d<<",threads="<<threads<<",sort_ms="<<sort_ms<<",group_rows="<<group_rows<<"\n";
  run_transform(g,w,bias,d,dp,threads,zt,yt);run_aggregate(g,w,bias,d,dp,threads,za,ya);run_fused(g,w,bias,perm,d,dp,threads,group_rows,zf,yf);
  error("aggregate_first",ya,yt);error("fused_amx",yf,yt);
  for(int r=-warmups;r<repeats;++r){auto a=run_aggregate(g,w,bias,d,dp,threads,za,ya);auto t=run_transform(g,w,bias,d,dp,threads,zt,yt);auto f=run_fused(g,w,bias,perm,d,dp,threads,group_rows,zf,yf);
