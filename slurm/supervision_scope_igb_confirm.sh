...
#SBATCH -t 01:00:00
#SBATCH --array=0-1%2
#SBATCH -J scope_igb_confirm
#SBATCH -o /online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905/runs/igb_confirm_%A_%a.out
#SBATCH -e /online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905/runs/igb_confirm_%A_%a.err

set -euo pipefail
root=/online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905
case "${SLURM_ARRAY_TASK_ID:-0}" in
  0) label=node_label_19.npy; out_dim=19; order=authority_active ;;
  1) label=node_label_2K.npy; out_dim=2983; order=q_first ;;
  *) exit 2 ;;
esac
...
export SCOPE_THREADS=32 SCOPE_LAYERS=2 SCOPE_WARMUPS=3 SCOPE_REPEATS=20
export SCOPE_OUT_DIM=$out_dim TFS_SCOPE_BACKWARD_ORDER=$order
stem=${label%.npy}
export SCOPE_OUTPUT="$root/runs/igb_scope_confirm_${stem}_d${out_dim}_${order}_l2_32t.json"
