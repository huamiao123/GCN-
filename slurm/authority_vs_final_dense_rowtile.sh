...
#SBATCH -t 00:50:00
#SBATCH --array=0-3%4
#SBATCH -J dense_rowtile
#SBATCH -o /online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905/runs/dense_rowtile_%A_%a.out
#SBATCH -e /online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905/runs/dense_rowtile_%A_%a.err

set -euo pipefail
root=/online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905
tiles=(50000 100000 150000 300000)
row_tile=${tiles[${SLURM_ARRAY_TASK_ID:?}]}
...
export TFS_COMPACT_DW_T4=1 TFS_SCOPE_NATIVE_LOGITS=1
export TFS_SCOPE_LOSS_ROW_TILE=$row_tile
export SCOPE_OUTPUT="$root/results/authority_vs_final_dense_rowtile/tile_${row_tile}.json"
export TFS_WORKER_CPUS="$(taskset -pc $$ | sed 's/.*: //')"
mkdir -p "$root/results/authority_vs_final_dense_rowtile"
