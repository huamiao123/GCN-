...
#SBATCH -t 00:45:00
#SBATCH --array=0-2
#SBATCH -J logsm_1024_rep
#SBATCH -o /online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905/runs/logsm_1024_rep_%A_%a.out
#SBATCH -e /online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905/runs/logsm_1024_rep_%A_%a.err

set -euo pipefail
root=/online1/huangjianqiang_group/hdacp1/wzh/tfs_supervision_scope_shadow_20260905
rep=${SLURM_ARRAY_TASK_ID:?}
module load intel/intel-oneapi-compilers/2024.1.0/gcc8.5.0-5ndeojp
while IFS='=' read -r variable _; do case "$variable" in TFS_*|HYBRID_*|SCOPE_*|IGB_*) unset "$variable" ;; esac; done < <(env)
...
export SCOPE_LAYERS=2 SCOPE_WARMUPS=2 SCOPE_REPEATS=10 SCOPE_OUT_DIM=1024 SCOPE_SYNTHETIC_LABELS=1
...
export SCOPE_OUTPUT="$root/results/logsoftmax_d1024_32t_replicates/rep${rep}.json"
export TFS_WORKER_CPUS="$(taskset -pc $$ | sed 's/.*: //')"
mkdir -p "$root/results/logsoftmax_d1024_32t_replicates"
numactl --cpunodebind=0-3 --localalloc python -u "$root/tests/bench_panelized_scope_igb.py"
