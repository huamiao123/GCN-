    ...
    def forward(self, x, graph):
        ...
        if (self._hs_cache is not None and
            ...
            if hs_cache is not None:
                ...
                # Use compact logical Hs for CSR forward; retain aligned Hs only
                # as the backward dW source when V2 padding is enabled.
...
if os.environ.get("HYBRID_CHECK") == "1":
    if os.environ.get("HYBRID_CHECK_DGL") == "1":
        ...
        # Arxiv node features are 128-dimensional; do not rely on the
        # shared wrapper's Products default (in_dim=100).
        dgl_model = DGLGCN(
    ...
    ref = ReferenceGCN(LAYERS, dropout=0.0, out_dim=OUT_DIM)
    hyb = HybridGCN(threads, LAYERS, dropout=0.0, in_dim=x.shape[1], out_dim=OUT_DIM,
                    num_nodes=x.shape[0])
    hyb.load_state_dict(ref.state_dict())
    # Input features are immutable in the authority workload.  Asking for
    # dX here would suppress the static first-layer cache and validate a
    # different execution plan from the timed training run.
...
if path == "hybrid":
    model, graph_arg = HybridGCN(threads, LAYERS, in_dim=x.shape[1], out_dim=OUT_DIM,
